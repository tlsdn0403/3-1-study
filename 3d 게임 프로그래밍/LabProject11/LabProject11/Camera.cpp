...
/*카메라 변환 행렬을 생성한다.
카메라의 위치 벡터, 카메라가 바라보는 지점, 카메라의 Up 벡터(로컬 y-축 벡터)를 파라메터로 사용하는
XMMatrixLookAtLH() 함수를 사용한다.*/ 
...
SpaceShipCamera::SpaceShipCamera(Camera * pCamera){
...
void SpaceShipCamera::Rotate(float fPitch, float fYaw, float fRoll){
	...
	if (m_pPlayer && (fPitch != 0.0f)) {
		...
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, PlayerPosition);
	...
	if (m_pPlayer && (fYaw != 0.0f)) {
		//플레이어의 로컬 y-축에 대한 y 각도의 회전 행렬을 계산한다.
...
ThirdPersonCamera::ThirdPersonCamera(Camera * pCamera){
	...
	if (pCamera) {
		...
		카메라의 Up 벡터를 월드좌표의 y-축이 되도록 한다.
		이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지
		3인칭 카메라(대부분 사람인 경우)의 로컬 y축 벡터가 월드좌표의 y-축이 되도록
		즉, 똑바로 서있는 형태로 설정한다는 의미이다. 
		그리고 로컬 x-축 벡터와 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다.
...
void ThirdPersonCamera::Update(XMFLOAT3 & xmf3LookAt, float fTimeElapsed){
	...
	if (m_pPlayer) {
		...
		카메라가 동시에 따라서 회전하지 않고 약간의 시차를 두고 회전하는 효과를 구현하기 위한 것이다.
		m_fTimeLag가 1보다 크면 fTimeLagScale이 작아지고 실제 회전(이동)이 적 게 일어날 것이다.
		m_fTimeLag가 0이 아닌 경우 fTimeElapsed를 곱하고 있으므로
		3인칭 카메라는 1초의 시간동안 (1.0f / m_fTimeLag)의 비율만큼 플레이어의 회전을 따라가게 될 것이다.*/
...
void ThirdPersonCamera::SetLookAt(XMFLOAT3 & xmf3LookAt){
	...
	XMFLOAT3 PlayerUpVector = m_pPlayer->GetUpVector();
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, PlayerUpVector);
