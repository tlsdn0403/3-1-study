...
void GameFramework::FrameAdvance(){
	...
	case GAME_1:
		...
		break;
	case WIN:
		if (m_pWinScene)m_pWinScene->Render(m_pd3dCommandList, m_pCamera);
		break;
	}
...
void GameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
		...
		case MENU:
			...
			break;
		case WIN:
			m_pWinScene->OnMouseClick(LOWORD(lParam), HIWORD(lParam)); // x, y 좌표 전달
			break;
		case GAME_1:
...
void GameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam){
	...
	case WM_KEYDOWN:
		...
		case'N':
			if (m_pGame_1_Scene) {
				ReleaseObjects();
				pGameState->CGameState::ChangeGameState(CGameState::GAME);
				BuildObjects();
			}
			break;
		case 'A':  
		...
		case'S':
			if (m_pScene && m_pPlayer) {
				m_pScene->ActivateShield(m_pPlayer.get(),1.0);
			}
		default:
			if (m_pScene)
...
void GameFramework::BuildObjects() {
    ...
    case TITLE:
        ...
        break;
	case WIN:
		if (m_pWinScene) m_pWinScene->ReleaseUploadBuffers();
		break;
	case MENU:
...
void GameFramework::AnimateObject(){
	...
	{
		if (m_pScene) {
			m_pScene->AnimateObjects(m_pPlayer.get(),m_GameTimer.GetTimeElapsed());
		}
		break;
	...
	case WIN:
		if (m_pWinScene) {
			m_pWinScene->AnimateObjects(m_GameTimer.GetTimeElapsed());
...
void GameFramework::ChoiceGameMode() // 모드에 따라 화면 출력
{
    ...
    break;
	case WIN:
		if (!m_pWinScene) m_pWinScene = new WinScene();
		m_pWinScene->Render(m_pd3dCommandList, m_pCamera);
		break;
    case MENU:
    if (!m_pMenuScene) 
    BuildObjects();
    ...
    BuildObjects();
		...
		if (m_pScene->CheckObjects()) {
			ReleaseObjects();
			pGameState->CGameState::ChangeGameState(CGameState::WIN);
			BuildObjects();
		}
	}
    break;
