	...
	: Mesh(pd3dDevice, pd3dCommandList)
{
	...
	// CPU용 정점 배열 저장
	m_pVertices = new DiffusedVertex[m_nVertices];
	memcpy(m_pVertices, vertices.data(), sizeof(DiffusedVertex)* m_nVertices);
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int
	nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : Mesh(pd3dDevice,
		pd3dCommandList)
{
